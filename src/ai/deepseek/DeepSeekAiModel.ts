...
import {DeepSeekModelConfig} from "./DeepSeekModelConfig.ts";
import {SseClient} from "../core/client/sse/SseClient.ts";
import {InnerEditor} from "../../core/AiEditor.ts";


export class DeepSeekAiModel extends AiModel {

    constructor(editor: InnerEditor, globalConfig: AiGlobalConfig) {
        super(editor, globalConfig, "deepseek");
        this.aiModelConfig = {
            endpoint: "https://api.deepseek.com",
            // model: "deepseek-chat",
            ...globalConfig.models?.deepseek
        } as DeepSeekModelConfig;
    }

    createAiClient(url: string, listener: AiMessageListener): AiClient {
        const config = this.aiModelConfig as DeepSeekModelConfig;
        const headers = {
